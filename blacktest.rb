...
def assert_if_blackjack_or_bust(player_total)
  if player_total == 21
    say "BLACKJACK!"
    @game_still_on = false
...
def dealer_winning?(player_total, dealer_total)
...
begin
  ...
  sleep 0.5
  puts "Your total is #{user_total}"
  assert_if_blackjack_or_bust(user_total)
  ...
  if @game_still_on
    while dealer_total<17
      ...
      describe_player_hand(dealer_hand)
      sleep 1
      calculate_hand_total(dealer_hand)
      puts "The dealer's total is #{dealer_total}"
      assert_if_blackjack_or_bust(dealer_total)
  ...
  if (@game_still_on) && (dealer_winning?(user_total, dealer_hand))
  ...
  if (@game_still_on) && (!dealer_winning?(user_total, dealer_hand))
    ...
    end until !@game_still_on || (dealer_winning?(user_total, dealer_total))

  if (dealer_winning?(user_total, dealer_total))
